...
public class JunctionApp extends Application {
    ...
    public static final String BROADCAST_STATUS = "edu.stanford.junction.sample.partyware.playlist.JunctionStatus";
	...
	public Uri currentNfcSharingUri(){
		if(this.mCurrentUri != null){
			...
			// builder.appendQueryParameter("jxuri", mCurrentUri.toString());
			// TODO: Update web's junction lib

			builder.appendQueryParameter("jxsessionid", mCurrentUri.getPath().substring(1));
			builder.appendQueryParameter("jxswitchboard", mCurrentUri.getAuthority());
				...
				public void run(){
					...
					try{
						...
						if(!isInterrupted()){
							...
							JunctionApp.this.mCurrentUri = Uri.parse(url.toString());
							SharedPreferences mPrefs = getSharedPreferences("prefs", MODE_PRIVATE);
							SharedPreferences.Editor ed = mPrefs.edit();
