...
public class MainActivity extends TabActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent() != null && getIntent().hasExtra("android.intent.extra.APPLICATION_ARGUMENT")) {
            ...
            jxUri = builder.scheme(JunctionApp.JUNCTION_SCHEME).build();
		         ...
		         getIntent().getData().toString().startsWith("http://prpl.stanford.edu")) {
		    Uri data = getIntent().getData();
		    String param = data.getQueryParameter("jxuri");
		    if (param != null) {
		        jxUri = Uri.parse(param);
		    } else {
		        param = data.getQueryParameter("jxsessionid");
		        String param2 = data.getQueryParameter("jxswitchboard");
		        if (param != null && param2 != null) {
		            jxUri = Uri.parse("junction://" + param2 + "/" + param);
		        }
		    }
		}
    ...
    protected void shareActivity(Uri uriIn){
        ...
        if(uri != null){
            NdefMessage ndef = getSharingNdef(uri);
            mNfc.share(ndef);
    ...
    public NdefMessage getSharingNdef(Uri uri) {
        Uri.Builder builder = Uri.parse("http://prpl.stanford.edu/junction/playlist").buildUpon();
        //builder.appendQueryParameter("jxuri", uri.toString());
        // TODO: Update web's junction lib
        builder.appendQueryParameter("jxsessionid", uri.getPath().substring(1));
        builder.appendQueryParameter("jxswitchboard", uri.getAuthority());

        return NdefFactory.fromUri(builder.build());
    }
}
